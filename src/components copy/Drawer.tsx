    ...
    <DrawerPortal>
      <DrawerOverlay>
        <DrawerPrimitives.Content
          ...
          className={cx(
            // base positioning
            "fixed right-0 top-0 h-screen bg-white dark:bg-[#090E1A] shadow-lg border border-gray-200 dark:border-gray-900",
            // full height and width
            "w-full sm:w-[90%] max-w-none",
            // animation states
            "data-[state=open]:translate-x-0 data-[state=closed]:translate-x-full transition-transform duration-300 ease-in-out",
            focusRing,
            className
          )}
          {...props}
...
const DrawerFooter = ({
  ...
  return (
    <div
      className={cx(
        "flex flex-col-reverse border-t border-gray-200  sm:flex-row sm:justify-end sm:space-x-2 dark:border-gray-900",
        className,
      )}
      {...props}
    />
