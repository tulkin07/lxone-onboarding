...
import { useUpdateHrDriverStatus } from "../../../hooks/useUpdateHrDriverStatus"
...
>({row}: DataTableRowActionsProps<TData>) {
   const { isPendingStatus:isPending, updateHrDriverStatus } = useUpdateHrDriverStatus()
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="min-w-40">
        ...
        {row?.original?.status != "accepted" && (
                    <DropdownMenuItem
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        updateHrDriverStatus({ id: row.original?.id, driver_status: "accepted" })
                      }}
                  ...
                  {row.original?.status != "rejected" && (
                    <DropdownMenuItem
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        updateHrDriverStatus({ id: row?.original?.id, driver_status: "rejected" })
                      }}
                      className="flex items-center gap-2"
                    >
