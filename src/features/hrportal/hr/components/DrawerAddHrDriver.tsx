...
import { useCreateHrDriver } from "../hooks/useCreateHrDriver"
...
export function DrawerAddHrDriver({
  open,
  closeDrawer,
  ownerCompanyId = 0, // Default to 0 as per JSON example
}: {
  open: boolean
  closeDrawer: () => void
  ...
  const { create, isPending } = useCreateHrDriver()
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const submissionData = {
      ...
      driver_license: formData.driver_license.split("/").pop() || "",
      driving_record: formData.driving_record.split("/").pop() || "",
      passport: formData.passport.split("/").pop() || "",
      owner_company_id: formData.owner_company_id,
      documents: formData.documents,
    }
    // submissionData.phone_number = "+998941302228"
    create(submissionData as unknown as HrDriverCreate, {
      onSuccess: () => {
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-3">
                    ...
                    <DatePicker
                      ...
                      onChange={(date) => {
                        if (date) {
                          setFormData((prev) => ({
                            ...prev,
                            birth_date: dayjs(date).format("YYYY-MM-DD HH:mm"),
                          }))
                        }
                      }}
                  ...
                  <div>
                    ...
                    <Input
                      type="text" // Changed to text to support leading zeros
                      name="zip_code"
                      ...
                      <DatePicker
                        ...
                        selected={
                          formData.license_expiration_date
                            ? new Date(formData.license_expiration_date)
                            : null
                        }
                        onChange={(date) => {
                          if (date) {
